...
class PetForm(forms.ModelForm):
    ...
    class Meta:
        model = models.Pet
        fields = ("name", "description", "city", "kind", "profile_picture", "size", "sex", "status")
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if state:
            self.fields["city"].choices += tuple(
                City.objects.filter(state__code=state).values_list("code", "name")
            )
...
class SearchForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["status"].choices += tuple(
            models.PetStatus.objects.values_list("id", "description")
            .filter(final=False)
