...
def pet_detail_view(request, pk_or_slug):
    ...
    context = {"pet": pet, "pet_url": request.build_absolute_uri(reverse("meupet:detail", args=[pet.slug]))}
...
def render_pet_list(request, kind, status, queryset):
    ...
    return render(
        request, "meupet/pet_list.html", {"pets": pets, "kind": kind, "status": status, "current_page": page}
    )
...
def pet_list(request, group, kind):
    ...
    return render(
        request,
        "meupet/pet_list.html",
        {"pets": pets, "kind": kind, "status": group.name, "current_page": page},
    )
...
class EditPetView(UpdateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.user == current_pet.owner:
            ...
            return HttpResponseRedirect(reverse("meupet:detail", kwargs={"pk_or_slug": current_pet.slug}))
